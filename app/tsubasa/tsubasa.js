...
import colors from 'colors'; // This may need to be replaced if used in a server context
...
export class Tsubasa {
    ...
    loadConfig() {
        ...
        const configPath = path.join(process.cwd(), 'config', 'config-tsubasa.json');
        try {
    ...
    async main(repeat_count,delay) {
        const dataFile = path.join(process.cwd(), 'config', 'data.txt');
